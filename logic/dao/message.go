...
func (*messageDao) Add(ctx *imctx.Context, message model.Message) error {
	_, err := ctx.Session.Exec("insert into message(message_id,user_id,sender_type,sender_id,sender_device_id,receiver_type,receiver_id,type,content,send_time,sequence) values(?,?,?,?,?,?,?,?,?,?,?)",
...
func (*messageDao) ListByUserIdAndSequence(ctx *imctx.Context, userId int64, sequence int64) ([]*model.Message, error) {
	rows, err := ctx.Session.Query("select id,message_id,user_id,sender_type,sender_id,sender_device_id,receiver_type,receiver_id,type,content,sequence,send_time,create_time from message where user_id = ? and sequence >= ?",
		userId, sequence)
	if err != nil {
		logger.Sugar.Error(err)
		return nil, err
