...
func (*userDao) Add(ctx *imctx.Context, user model.User) (int64, error) {
	result, err := ctx.Session.Exec("insert ignore into user(number,nickname,sex,avatar,password) values(?,?,?,?,?)",
...
func (*userDao) Get(ctx *imctx.Context, id int64) (*model.User, error) {
	row := ctx.Session.QueryRow("select number,nickname,password,sex,avatar from user where id = ?", id)
	user := new(model.User)
...
func (*userDao) GetByNumber(ctx *imctx.Context, number string) (*model.User, error) {
	row := ctx.Session.QueryRow("select id,number,nickname,password,sex,avatar from user where number = ?", number)
	user := new(model.User)
