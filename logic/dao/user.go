...
func (*userDao) Add(ctx *imctx.Context, user model.User) (int64, error) {
	result, err := ctx.Session.Exec("insert ignore into user(app_id,user_id,nickname,sex,avatar_url,extra) values(?,?,?,?,?,?)",
...
func (*userDao) Get(ctx *imctx.Context, appId, userId int64) (*model.User, error) {
	row := ctx.Session.QueryRow("select nickname,sex,avatar_url,extra,create_time,update_time from user where app_id = and user_id = ?",
		appId, userId)
	user := model.User{
		AppId:  appId,
		UserId: userId,
