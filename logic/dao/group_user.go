...
func (*groupUserDao) Get(ctx *imctx.Context, appId, groupId, userId int64) (*model.GroupUser, error) {
	...
	err := ctx.Session.QueryRow("select label,extra from group_user where app_id = ? and group_id = ? and user_id = ?",
...
func (*groupUserDao) Add(ctx *imctx.Context, appId, groupId, userId int64, label, extra string) error {
	_, err := ctx.Session.Exec("insert ignore into group_user(app_id,group_id,user_id,label,extra) values(?,?,?,?,?)",
